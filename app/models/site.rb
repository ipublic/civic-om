...
class Site < CouchRest::Model::Base
  ...
  property :label, String, :alias => "name"
  ...
  validates_exclusion_of :subdomain, in: Subdomain::BLACK_LIST, message: "is not available"
  validates_uniqueness_of :subdomain, :view => 'all', message: "is not available"
  ...
  def label
    @label ||= @term.subdomain unless @subdomain.nil?
  end
...
private
  def init_site
    ...
    write_attribute(:identifier,  %W[#{class_basename} #{subdomain}].join('_'))
